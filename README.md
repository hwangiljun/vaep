...
이 행동이 우리 팀의 가까운 미래 득점 확률을 얼마나 올리고 실점 확률을 얼마나 낮췄냐로 바꿔서, 패스·드리블·태클·클리어링 같은 모든 온더볼 액션에 점수를 매기는 방법이 VAEP다. 

골과 어시스트만 세면 보이지 않던 기여들이 보여진다.

StatsBomb이 공개한 **라리가 2015/16 380경기**로 [socceraction](https://github.com/ML-KULeuven/socceraction)의
VAEP 파이프라인을 처음부터 끝까지 다시 돌린 기록이다. 
원본 이벤트 → SPADL 행동 757,309개 →
게임 상태 피처 145개 → 득점·실점 확률 모델 → 행동별 가치 → 선수 랭킹 순으로 간다.

과정과 결과는 **[`vaep.ipynb`](vaep.ipynb)** 에 다 들어 있다(출력까지 저장해 뒀으니 열기만 하면 된다).
— `python run_all.py`.
...
값은 모두 **그 경기를 학습에 쓰지 않은 모델**의 예측에서 보여진다. 즉 자기 경기를 외운 모델이 준 점수가 아니다.

### 한 번의 공격

`figures/sequence.png`는 골로 끝난 공격 하나를 행동 단위로 쪼갠 것이다.
셀타 비고가 자기 진영에서 태클로 공을 끊어 10개 행동 만에 득점하는 동안,
득점 확률이 0.5% → 3.9% → 7.3% → 10.5% → 골 확률이 올라간다.
...
Python 3.11.13에서 테스트했다 (3.9 ~ 3.12면 가능).
...
여기까지 오는 데 걸림돌이 존재했다. **socceraction 1.5.3에는 위치가 없는 이벤트의 좌표가
초기화되지 않은 메모리 값으로 남는 버그가 존재한다** (`spadl/statsbomb.py`의 `_convert_locations`가
